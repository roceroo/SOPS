...
<div id="app">
	...
	include('config.php');
		...
		if($qtd > 0){
			print "<table class='table table-striped table-hover' style='background: #1a1a1a; font-weight: 100; width: 80%; margin: 20vh auto'>";
			print "<tr>";
			print "<th>Marca do Veículo</th>";
			print "<th>Modelo do Veículo</th>";
			print "<th>Ano de Fabricacão</th>";
			print "<th>Placa</th>";
			print "<th>Ações</th>";
			print "</tr>";
			while($row = $res->fetch_object()){
				...
				print "<td>".$row->ano_fabricacao."</td>";
				print "<td>R$".$row->placa."</td>";
				print "<td>
						<button class='btn btn-success' onclick=\"location.href='?page=editar-veiculo&id_veiculo=".$row->id_veiculo."';\">Editar</button>
						
						<button class='btn btn-danger' onclick=\"if(confirm('Tem certeza que deseja excluir?')){location.href='?page=salvar-veiculo&acao=excluir&id_veiculo=".$row->id_veiculo."';}else{false;}\">Excluir</button>
					</td>";
				print "</tr>";
			}
			print "</table>";
